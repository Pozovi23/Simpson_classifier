...
def train(model, device, train_loader, val_loader, path_to_save_model, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for image, label in train_loader:
            ...
            writer.add_scalar("Loss/train each batch", loss.item(), num_of_train_batches)
        ...
        for image, label in val_loader:
            with torch.no_grad():
                ...
                output = model(image)
                loss = criterion(output, label)
                writer.add_scalar("Loss/validation each batch", loss.item(), num_of_validation_batches)
                total_validation_loss += loss.item()
                num_of_validation_batches += 1
                y_true.extend(label.cpu().numpy())
                y_pred.extend(output.cpu().numpy().argmax(-1))

        writer.add_scalar("Loss/validation", total_validation_loss / num_of_validation_batches, epoch)
